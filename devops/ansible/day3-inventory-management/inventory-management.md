...
#### 概念解析
- **主机列表**: 可以是主机名、IP 地址或域名，支持通配符和范围表达式
- **组织结构**: 通过组（group）实现逻辑分类，支持组嵌套和层次结构
- **变量存储**: 支持主机级变量、组级变量，实现配置差异化
- **连接信息**: 包括 SSH 端口、用户名、认证方式等连接参数
...
#### 重要性解析
- **目标定位**: 通过主机模式（host patterns）精确控制任务执行范围
- **逻辑分组**: 基于业务功能、环境、地域等维度进行分组管理
- **环境隔离**: 实现配置隔离，避免环境间的干扰和误操作
- **配置管理**: 通过变量系统实现"一次编写，多处使用"的配置管理
...
#### 语法解析
- **`web1.example.com`**: 简单主机定义，使用默认连接参数
- **`web2.example.com ansible_host=192.168.1.10`**: 主机别名，`ansible_host`指定实际IP
- **`[webservers]`**: 组定义，`[]`内为组名，下面列出组成员
- **`db[01:03].example.com`**: 范围表达式，生成 db01、db02、db03 三个主机名
- **`[webservers:vars]`**: 组变量定义，`:vars`后缀表示变量块
- **`[production:children]`**: 组嵌套，`:children`表示包含其他组
- **INI 格式特点**: 简洁直观，适合简单场景，但嵌套结构有限
