...
**第一阶段：基础概念（1-2天）**
1. 理解变量和模板的基本概念
2. 学会在 Playbook 中定义和使用简单变量
3. 掌握 `debug` 模块查看变量值

**第二阶段：模板入门（2-3天）**
1. 创建第一个模板文件（`.j2` 文件）
2. 学习变量插值语法 `{{ variable }}`
3. 使用 `template` 模块生成配置文件

**第三阶段：高级功能（3-5天）**
1. 掌握条件语句 `{% if %}` 和循环 `{% for %}`
2. 学习使用过滤器 `{{ var | filter }}`
3. 理解变量优先级和作用域
...
```
错误：变量 'app_name' 未定义
解决：在 vars 中定义变量，或使用默认值 {{ app_name | default('myapp') }}
```

**错误2：模板文件找不到**
```
错误：模板文件不存在
解决：确保模板文件放在 templates/ 目录下
```

**错误3：权限问题**
```
错误：权限被拒绝
解决：检查 dest 路径的权限，或使用 become: yes 提升权限
```
...
**新手记住这几点：**
1. **变量是值的容器**：就像编程中的变量一样
2. **模板是带占位符的文档**：Ansible 会用实际值替换占位符
3. **从简单开始**：先掌握基本变量和模板，再学习高级功能
4. **多练习**：通过实际项目加深理解
...
**学习建议：** 每天花1-2小时练习，一周内就能熟练掌握 Ansible 变量和模板的使用！